...
module Prick::Lang
  class Parser < CompilerProcess
    ...
    # Absolute path to starting source file (#settings.source_file is not
    # required to be an absolute path)
    forward_to :compiler, :source_file

    # The AST generated by #parse
    attr_reader :ast

    # Forward #token and #peek_token
  ...
  private
    ...
    def parse_program
      token = ProgramToken.new(source_file, 1, 1)
      file = Ast::SourceFile.new(token, path: source_file)
    ...
    def parse_file
      if peek.extname == Prick::SOURCE_EXT
        parse_prick_file(peek.path)
      else
        command = Ast::FileCommand.new peek
        command.file = Ast::File.new(read, dir: @tokenizer.dir)
        command
      end
    end

    def parse_dir
      parse_prick_file File.join(peek_token.path, Prick::MAKE_FILENAME)
    end
...
#         command.source = Ast::Source.new readtext(limit)&.text
    ...
    def parse_value
      ...
      case token.kind
        ...
        when :FILE, :DIR; Ast::File.new(token, dir: @tokenizer.dir)
    ...
    # make_file(token, path)
    # make_file(token, nil, dir)
    #
    def make_file(token, path = nil, dir = nil)
    end
